...
regexp* regcomp(const char* exp)
{
  ...
  if (exp == NULL) FAIL("NULL argument to regcomp");
 ...
 */
static char*          reg(cp, paren, flagp)
...
int*                  flagp;
{
  ...
  *flagp = HASWIDTH; /* Tentatively. */
  ...
  if (paren) {
    /* Make an OPEN node. */
    if (cp->regnpar >= NSUBEXP) FAIL("too many ()");
    parno = cp->regnpar;
    cp->regnpar++;
 ...
 */
static char*          regbranch(cp, flagp)
register struct comp* cp;
int*                  flagp;
{
  register char* ret;
...
int StrInsert(char** p_string, char* insert_string, size_t index, int need_realloc)
{
  ...
  if (index > length)
    ...
    if (need_realloc == 1) *p_string = realloc(*p_string, length + length1 + 1);
...
int StrDelete(char** p_string, size_t from, size_t length, int need_realloc)
{
  ...
  if (from + length > total_length)
    del_length = total_length - from;
  else
    del_length = length;

  memmove(*p_string + from, *p_string + from + del_length, total_length - del_length);
  if (need_realloc == 1) *p_string = realloc(*p_string, total_length - del_length + 1);
